	...
	public void start() {
		...
		for (int i = 1; i <= 1; i++) {
			...
			for (Item item : items) {
				...
				item.setImage("http://image.yxg.com/" + newName);
				...
				if (desc != null) {
					...
					for (Element element : imgs) {
						String imgUrl = element.hasAttr("data-lazyload") ? element
								.attr("data-lazyload") : element.attr("src");
						String newName2 = StringUtils.replace(UUID.randomUUID()
								.toString(), "-", "")
								+ "." + StringUtils.substringAfterLast(imgUrl, ".");
						urlMapping.put("http:" + imgUrl, newName2); // http:
																	// 自己家的防止报错
						find.add(imgUrl);
						replace.add("http://image.yxg.com/" + newName2);
					}
			...
			// 启动新线程下载图片
			Runnable runnable = new ImageDownloadCrawler(urlMapping);

			ThreadPool.runInThread(runnable);
