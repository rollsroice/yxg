...
public class JD3CCrawler extends BaseCrawler {
	 ...
	 * 
	 * @param baseUrl
	 *            like http://list.jd.com/list.html?cat=737,794,798&page={page}
	 */
	...
	protected Collection<Item> doParser(String html) {
		...
		for (Element li : lis) {
			...
			String id = li.select(".gl-i-wrap").attr("data-sku");

			// String title = li.select(".p-name").text();
			String title = li.select(".p-name a em").text();

			// String image = li.select(".p-img img").attr("data-lazy-img");
			String imgage = li.select(".gl-i-wrap .p-img").select("img").attr("src");

			String imgage1 = li.select(".gl-i-wrap .p-img").select("img").attr("data-lazy-img");
			...
			// item.setImage(image);
			if (!imgage.equals("")) {
				item.setImage("http:" + imgage);
			} else {
				item.setImage("http:" + imgage1);
			}
